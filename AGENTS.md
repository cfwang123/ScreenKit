...
8. 用脚本对项目代码进行大规模替换、编辑时，把所有代码文件打包为7z，放到tmp/back/下备份。非代码文件可以不备份。
9. 代码风格指导（按需读取，动手改代码前先对照）
...
11. 同一个问题反复修改还不行时，用命令行/adb+日志自己调试
	...
	- 安卓：adb + logcat，模拟运行。
	- 模拟问题场景，观察日志，定位根因后再改。
12. 修改功能时，同时更新当前工作目录下的 `README.md` 与 `CHANGELOG.md`（`CHANGELOG.md` 章节标题：软件有版本号则用「版本号 + 日期范围」，如 `## v1.2.3 (2026-06-01 ~ 03)`（同一版本跨多日可写起止日；单日可写 `## v1.2.3 (2026-06-01)`）；无版本号则仅用日期，如 `## YYYY-MM-DD`；本次改动写入对应章节，无则新建）。
