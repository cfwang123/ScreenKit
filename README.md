...
2. Phone **传文件/投屏** → **投屏** (uses the PC already selected on the file-transfer page) → allow capture / mic / notifications → **Scan** or **My PCs** → **Start**. A checked list row is not required if a host is already chosen. The phone sends `hello` and waits for the PC’s `hello` before capture; the PC viewer opens only after that reply. Wi‑Fi media uses HTTP `/cast` (default 1224). USB accessory reads with a short timeout so the viewer is not left black after hello. If the PC app is closed or does not reply, the phone shows **电脑未打开 ScreenKit**. After ~90s without an accessory it falls back to USB tethering. **USB 投屏(adb)** uses `adb reverse` and needs USB debugging. USB-only test pattern: `scst_usb_pat`.
