...
2. Phone launcher **投屏** → allow capture / mic / notifications → **Scan** → pick the PC → **Start**. Or type an IP. USB without adb reverse: on the PC open **Tools → Screencast** and click **USB配件** (helper switches AOA via WinUSB/ADB; if spacedesk owns the accessory interface it force-binds inbox WinUSB, then bridges bulk; the UI process never calls LibUsb). The phone can also auto-start when the accessory attaches. After ~90s without an accessory it falls back to USB tethering. **USB 投屏(adb)** uses `adb reverse` and needs USB debugging. USB-only test pattern (no capture): `scst_usb_pat`.
