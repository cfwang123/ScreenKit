...
6. **Web manager** (same HTTP port): **File sync → Web manager** shows the LAN URL, QR, and login password. Desktop page `/`, phone page `/m` (white UI, SVG icons). Sign in to upload, mkdir, rename, delete; optional **Stay signed in**. Open subfolders; zip a folder or the selection. A correct `/f/相对路径` URL downloads without login. The standalone interactive mobile redesign prototype is `ScreenKit/SendFile/web/m-prototype.html`; open it directly in a browser (add `?login=1` to preview the login screen).
