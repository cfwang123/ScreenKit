...
| **Install features** | Feature tree checks what’s already installed; add (green) / remove (red) sizes; Reset; confirm to pick components. Voices on a separate tab. CN mirrors when locale is Chinese. |
...
2. Later: **Tools → Install features**
   - **Features**: installed items are checked; add (green) / remove (red) counts and sizes; Reset restores. Confirm to check matching components. Voices are not on this tab.
