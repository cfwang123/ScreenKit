...
2. Phone launcher **投屏** → allow capture / mic / notifications → **Scan** → pick the PC → **Start**. The phone sends `hello` and waits for the PC’s `hello` before capture or 投屏中; the PC viewer opens only after that reply. Or type an IP. USB without adb reverse: ScreenKit must be running (receive starts the AOA helper; **USB配件** still works). If the PC app is closed or does not reply, the phone shows **电脑未打开 ScreenKit**, not 投屏中. If spacedesk owns the accessory interface it force-binds inbox WinUSB, then bridges bulk; the UI process never calls LibUsb. The phone can also auto-start when the accessory attaches. After ~90s without an accessory it falls back to USB tethering. **USB 投屏(adb)** uses `adb reverse` and needs USB debugging. USB-only test pattern (no capture): `scst_usb_pat`.
