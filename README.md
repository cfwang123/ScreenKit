...
3. A view window opens on the monitor under the mouse. **Tab** shows type (network / usb / adb), resolution, fps, bitrate, latency. Closing the window disconnects. Quality changes apply live. Landscape re-encodes at the same short-edge quality (does not crop a portrait frame). Stopping on the phone closes the PC window immediately (`GET /api/cast/stop`).
