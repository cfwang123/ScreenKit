...
- **Tools → Network tools**: Ping, DNS lookup, domain/IP WHOIS, traceroute, ping-locate (RTT to worldwide nodes plus HTTP geolocation via the Windows system proxy). Each action clears previous output. Tray and CLI `--test-nettool`.
...
- **工具 → 网络工具**：Ping、域名解析、域名/IP WHOIS、路由跟踪、Ping 定位（全球节点 RTT，以及经 Windows 系统代理的 HTTP 出口定位）。每次点按钮清空旧输出。托盘与 CLI `--test-nettool`。
