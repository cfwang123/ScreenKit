...
- **Tools → Network tools**: Ping, DNS lookup, domain/IP WHOIS, traceroute, ping-locate, and proxy-locate (HTTP geo via the Windows system proxy). Each action clears previous output. Tray and CLI `--test-nettool`.
...
- **工具 → 网络工具**：Ping、域名解析、域名/IP WHOIS、路由跟踪、Ping 定位、代理定位（系统代理 HTTP 出口）。每次点按钮清空旧输出。托盘与 CLI `--test-nettool`。
