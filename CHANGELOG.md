...
- HTTP `POST /api/chat`: text or audio user message, text reply, optional TTS (`auto_tts`/`tts`) returning `wav_base64` (omitted → config `chat_auto_tts`). Status `llm_chat`. CLI: `ScreenKit --test-http-chat`.
...
- HTTP `POST /api/chat`：文本或语音用户消息、文本回复；`auto_tts`/`tts` 控制是否返回 `wav_base64`（未传则用配置 `chat_auto_tts`）。状态字段 `llm_chat`。CLI：`ScreenKit --test-http-chat`。
