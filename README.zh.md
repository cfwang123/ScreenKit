...
**当前版本：1.0.8** · [GitHub Releases](https://github.com/cfwang123/ScreenKit/releases/latest)
...
| 文件 | 说明 |
|------|------|
| [`screenkit_1.0.8.7z`](https://github.com/cfwang123/ScreenKit/releases/latest) | Windows x64 精简包（exe + 托管依赖）。模型与运行库在程序内安装。 |
| `screenkit1.0.8.apk` | 局域网文件/文本同步安卓客户端（同一发布页，或电脑 **文件同步 → 安装到手机**）。 |
...
| **图片格式转换** | 菜单 **工具 → 图片格式转换**：批量转 JPG/PNG/BMP，可限制最大宽高、旋转/镜像；列表支持图片视图（小缩略图）与列表视图；选中后预览已应用质量与变换的效果；输出到源文件旁 `output/` 或指定目录。可选：压缩后体积仍 ≥ 原图 N% 时用原图（默认 80%；有旋转或实际缩小时仍用新图）。 |
| **二维码 / 条码** | **工具 → 二维码 / 条码生成**：QR、Data Matrix、Code 128/39、EAN、UPC。图下显示一行原文。编码 UTF-8、GBK 或 Hex 二进制（默认 UTF-8）。 |
| **批量重命名** | **工具 → 批量重命名**：源文件列表（添加/删除/上下移动，点列头按名称、修改或添加日期排序）；目标名用文本框可手改。Everything 风格表达式（`%1`、`#` / `###`）；改列表或规则时自动重算。 |
...
3. 首次连接电脑弹窗确认（主窗在托盘时仍能弹出）。
4. 手机 App 已连接时，电脑拖入或粘贴文件/文件夹/图片发到手机绑定文件夹；无连接则不传。手机分享/上传一律写到电脑 `sendfile/`。
...
**工具 → 图片格式转换**：拖入文件、文件夹或图片；目标 JPG / PNG / BMP；可选最大宽高（与截图相同的等比缩小）；选中项可旋转 90/180/270° 或镜像。**图片**视图显示小缩略图（可切回 **列表**）。右侧预览已应用质量与变换后的效果。一键转换输出到每个源文件旁的 `output/` 或指定目录。
