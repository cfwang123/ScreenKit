...
| **剪贴板** | 粘贴图片识别；复制图片 / 文字 / 路径（「编辑」菜单：复制的图无源文件时先存 `screenshots/` 再复制路径文本）；菜单/托盘可切换截图完成时「复制为图片 / 文件 / 路径」，切换时会按新方式重写上次截图到剪贴板；复制为路径用 persist 的 OLE 文本（丢掉延迟位图，避免粘成「▀」或 Flush 后剪贴板变空） |
| **文字叠加** | 图上叠加文字层，拖选复制 |
