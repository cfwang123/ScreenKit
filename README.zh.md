...
- `[ui]`：热键、托盘、界面语言、`update_check_days`（启动时自动检查更新间隔，默认 7 天，0=不自动检查；菜单「检查更新」不受限）、`http_proxy` / `http_proxy_addr`（访问 GitHub 等非中国网站时的 HTTP 代理，国内镜像直连）、`capture_log`（`log/capture.log`：多屏/DPI，以及截图落盘 prep/encode/clip 耗时，≥500ms 标 `SLOW`）、`llm_log`（`log/llm.log`，润色请求/响应，不含 key）、`screenshot_keep_days`（截图历史保留天数，默认 3，0=不限；仅启动时后台清理，截图时不删）  
