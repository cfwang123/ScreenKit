...
- `[ui]`：热键、托盘、界面语言、`update_check_days`（启动时自动检查更新间隔，默认 7 天，0=不自动检查；菜单「检查更新」不受限）、`http_proxy` / `http_proxy_addr`（访问 GitHub 等非中国网站时的 HTTP 代理，国内镜像直连）、`capture_log`、`llm_log`（`log/llm.log`，润色请求/响应，不含 key）  
