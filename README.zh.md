...
| **投屏** | **工具 → 投屏**（托盘「工具」同样入口）：局域网 / USB 接收手机或另一台电脑画面，也可把本机投出。画质 540p/720p/1080p，可关声音。画面窗不置顶；画面可选适应窗口或铺满窗口。发现与传文件共用 UDP 17531，画面走 HTTP `/cast`（默认 1224）。USB 非 adb reverse：电脑开接收即自动配件助手。或 **USB 投屏(adb)**（需 USB 调试）。手机 App 桌面有独立「投屏」图标。 |
...
2. 手机点桌面 **投屏** → 允许截屏 / 麦克风 / 通知 → **扫描** → 选中电脑 → **开始投屏**。也可填 IP。手机先发 hello，收到电脑 hello 才开始采集并显示「投屏中」；电脑回 hello 后才弹窗。电脑未开或未应答则提示 **电脑未打开 ScreenKit**，不会显示投屏中。扫描走 Wi‑Fi（插 USB 时也会绑 Wi‑Fi 口并向上次电脑单播）。不用 adb reverse 的 USB：电脑必须已开 ScreenKit（开接收会自动配件助手；也可点 **USB配件**）。配件口 `MI_00` 若被 spacedesk 占用，会换成 WinUSB（不对 spacedesk 设备 Open），再桥接 bulk。主程序不碰 LibUsb。配件连上时手机会自动开投屏。约 90 秒没有配件则改走 USB 网络共享。**USB 投屏(adb)** 走 `adb reverse`，需要 USB 调试。纯 USB 测试画面（不截屏）走配件 bulk：`scst_usb_pat`。
