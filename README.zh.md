...
| 模块 | 说明 |
|------|------|
| **PC 文件传输** | 与 HTTP API 共用端口（默认 `1224`）+ UDP 发现 `17531`。主界面 **文件同步** Tab：上表平铺浏览程序旁 `sendfile/`（不进子目录），可框选、剪切/复制/粘贴、删到回收站、拖到资源管理器。下方拖入在手机已连接时发到手机绑定文件夹；手机分享写到电脑 `sendfile/`。**安装到手机** 显示局域网地址和二维码。**网页管理**（同一端口）：电脑 `/`、手机 `/m`（白底、淡色点缀、按钮带 SVG 图标），登录后上传/管理，下载只需 `/f/…`。配套 App：`android/`（`com.whj.screenkit`，桌面「传文件」「投屏」）。 |
...
| **投屏** | **工具 → 投屏**（托盘「工具」同样入口）：局域网 / USB 接收手机或另一台电脑画面，也可把本机投出。画质 540p/720p/1080p，可关声音。画面窗不置顶；画面可选适应窗口或铺满窗口。发现 UDP 19518、媒体 TCP 19519。USB 非 adb reverse：电脑开接收即自动配件助手。或 **USB 投屏(adb)**（需 USB 调试）。手机 App 桌面有独立「投屏」图标。 |
...
6. **网页管理**（同一 HTTP 端口）：**文件同步 → 网页管理** 显示局域网地址、二维码和登录密码。电脑版 `/`，手机版 `/m`（白底、淡色点缀、按钮带 SVG 图标）。登录后可上传、建目录、进入子文件夹、改名、删除、打包 zip；可选 **保持登录**。正确的 `/f/相对路径` 即可下载，无需登录。手机端重设计交互原型位于 `ScreenKit/SendFile/web/m-prototype.html`，可直接用浏览器打开；追加 `?login=1` 可预览登录页。原型无标题栏和搜索栏，采用 100% 宽度紧凑列表，底部为上传、拍照、新建；点击进入/下载，长按进入多选。

安卓端说明：[android/README.md](android/README.md)。

### 投屏（电脑 ↔ 安卓 / 电脑 ↔ 电脑）

1. 电脑 **工具 → 投屏**（默认启动即接收；参数设置 → 接口可关）。同一 Wi‑Fi，不要访客隔离。
2. 手机点桌面 **投屏** → 允许截屏 / 麦克风 / 通知 → **扫描** → 选中电脑 → **开始投屏**。也可填 IP。不用 adb reverse 的 USB：电脑必须已开 ScreenKit（开接收会自动配件助手；也可点 **USB配件**）。电脑软件未开时，USB 投屏会提示 **电脑未打开 ScreenKit**，不会显示投屏中。配件口若被 spacedesk 占用会换成系统 WinUSB，再桥接 bulk。主程序不碰 LibUsb。配件连上时手机会自动开投屏。约 90 秒没有配件则改走 USB 网络共享。**USB 投屏(adb)** 走 `adb reverse`，需要 USB 调试。纯 USB 测试画面（不截屏）走配件 bulk：`scst_usb_pat`。
3. 电脑弹出无边框画面窗；**Tab** 显示分辨率、帧率、码率、延时。关闭画面即断开。画质档可即时改。横屏会按同一画质短边重编码，不把竖屏画面裁小。
