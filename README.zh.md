...
2. 手机点桌面 **投屏** → 允许截屏 / 麦克风 / 通知 → **扫描** → 选中电脑 → **开始投屏**。也可填 IP。不用 adb reverse 的 USB：电脑打开 ScreenKit 接收后会自动开配件助手；也可在 **工具 → 投屏** 点 **USB配件**。配件口若被 spacedesk 占用会换成系统 WinUSB，再桥接 bulk。主程序不碰 LibUsb。配件连上时手机会自动开投屏。约 90 秒没有配件则改走 USB 网络共享。**USB 投屏(adb)** 走 `adb reverse`，需要 USB 调试。纯 USB 测试画面（不截屏）走配件 bulk：`scst_usb_pat`。
