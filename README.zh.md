...
| **投屏** | **工具 → 投屏**（托盘「工具」同样入口）：局域网 / USB 接收手机或另一台电脑画面，也可把本机投出。画质 540p/720p/1080p，可关声音。发现 UDP 19518、媒体 TCP 19519。无网络 USB：电脑点 **USB配件**，手机点 **USB 投屏**（AOA bulk，不用调试）。或 **USB 投屏(adb)**（需 USB 调试）。手机 App 桌面有独立「投屏」图标。 |
...
2. 手机点桌面 **投屏** → 允许截屏 / 麦克风 / 通知 → **扫描** → 选中电脑 → **开始投屏**。也可填 IP。无网络 USB：电脑 **工具 → 投屏** 点 **USB配件**（独立进程切 AOA 并桥接，主程序不碰 LibUsb），插上数据线，手机点 **USB 投屏** 并允许配件。12 秒内没有配件则改走 USB 网络共享。**USB 投屏(adb)** 需要 USB 调试。
