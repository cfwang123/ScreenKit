...
6. **网页管理**（同一 HTTP 端口）：**文件同步 → 网页管理** 显示局域网地址、二维码和登录密码。电脑版 `/`，手机版 `/m`（白底、淡色点缀、按钮带 SVG 图标）。登录后可上传、建目录、进入子文件夹、改名、删除、打包 zip；可选 **保持登录**。正确的 `/f/相对路径` 即可下载，无需登录。手机端重设计交互原型位于 `ScreenKit/SendFile/web/m-prototype.html`，可直接用浏览器打开；追加 `?login=1` 可预览登录页。原型采用 100% 宽度紧凑列表，点击文件夹进入、点击文件下载，长按进入多选并显示操作栏。
