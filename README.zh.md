...
2. 手机 **传文件/投屏** → **投屏**（会带上已选/已连接的电脑）→ 允许截屏 / 麦克风 / 通知 → **扫描** 或 **我的电脑** → **开始投屏**。列表不必保持勾选，已选主机或手动 IP 即可开始。画面走 HTTP `/cast`（默认 1224）。USB 配件短超时读，避免弹窗后一直黑屏。电脑未开或未应答则提示 **电脑未打开 ScreenKit**。约 90 秒没有配件则改走 USB 网络共享。**USB 投屏(adb)** 走 `adb reverse`，需要 USB 调试。纯 USB 测试画面：`scst_usb_pat`。
