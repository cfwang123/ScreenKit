...
| 模块 | 说明 |
|------|------|
| **PC 文件传输** | 与 HTTP API 共用端口（默认 `1224`）+ UDP 发现 `17531`。主界面 **文件同步** Tab：上表平铺浏览程序旁 `sendfile/`（不进子目录），可框选、剪切/复制/粘贴、删到回收站、拖到资源管理器。下方拖入在手机已连接时发到手机绑定文件夹；手机分享写到电脑 `sendfile/`。**安装到手机** 显示局域网地址和二维码。**网页管理**（同一端口）：电脑 `/`、手机 `/m`（紧凑列表、长按多选、上传/拍照/新建），登录后上传/管理，下载只需 `/f/…`。配套 App：`android/`（`com.whj.screenkit`，桌面「传文件」「投屏」）。 |
...
6. **网页管理**（同一 HTTP 端口）：**文件同步 → 网页管理** 显示局域网地址、二维码和登录密码。电脑版 `/`，手机版 `/m`。登录后可上传、建目录、改名、删除等；可选 **保持登录**。手机端：面包屑路径、紧凑列表（点击进入/下载，长按多选），底部 **上传 / 拍照 / 新建**，多选后可下载、ZIP、复制链接、改名、删除。正确的 `/f/相对路径` 即可下载，无需登录。交互参考：`ScreenKit/SendFile/web/m-prototype.html`。
...
3. 电脑弹出无边框画面窗；**Tab** 显示类型（网络 / usb / adb）、分辨率、帧率、码率、延时。关闭画面即断开。画质档可即时改。横屏会按同一画质短边重编码，不把竖屏画面裁小。手机停止投屏时电脑窗口立刻关闭（`GET /api/cast/stop`）。
