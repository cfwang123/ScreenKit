...
6. **网页管理**（同一 HTTP 端口）：**文件同步 → 网页管理** 显示局域网地址、二维码和登录密码。电脑版 `/`，手机版 `/m`。登录后可上传、建目录、改名、删除等；可选 **保持登录**。手机端：面包屑路径、紧凑列表（点击进入/下载，长按多选），底部 **上传 / 拍照 / 新建**，多选后可下载、ZIP、复制链接、改名、删除；账户里可 **检查更新**（经电脑查询 GitHub 最新 APK）。正确的 `/f/相对路径` 即可下载，无需登录。交互参考：`ScreenKit/SendFile/web/m-prototype.html`。

安卓端说明：[android/README.md](android/README.md)。

### 投屏（电脑 ↔ 安卓 / 电脑 ↔ 电脑）

1. 电脑 **工具 → 投屏**（默认启动即接收；参数设置 → 接口可关）。同一 Wi‑Fi，不要访客隔离。启动本程序会结束其它目录的旧 ScreenKit 以及占用 TCP 19519 的进程。若 19519 被已退出进程的套接字占死，会改听 19520+，扫描列表带实际端口。
