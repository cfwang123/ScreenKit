...
sealed class OcrEngine : IDisposable {
	...
	void recognbatch(
		...
		for (int bi = 0; bi < count; bi++) {
			var (text, score) = ctcdecodeplane(data, bi * plane, tLen, cLen, timeLast);
	...
	(string text, float score) ctcdecodeplane(float[] data, int offset, int t, int c, bool timeLast) {
		...
		for (int i = 0; i < t; i++) {
			...
			if (best == blank) { prev = blank; continue; }
			if (best == prev) continue;
			prev = best;
			var idx = best - 1;
			if ((uint)idx < (uint)charset.Length) {
		...
		var score = scoreN > 0 ? scoreSum / scoreN : 0f;
		if (score > 1.5f || score < 0f) {
			// 未归一 logits → sigmoid 均值（与旧逻辑一致）
			if (scoreN == 0) score = 0f;
			else {
				// 重新扫一遍过重；仅在罕见路径触发
				scoreSum = 0;
				scoreN = 0;
				...
				for (int i = 0; i < t; i++) {
					...
					if (best == prev) continue;
					prev = best;
					if (best >= 1 && best - 1 < charset.Length) {
						scoreSum += 1f / (1f + MathF.Exp(-bestV));
						scoreN++;
					}
				}
				score = scoreN > 0 ? scoreSum / scoreN : 0f;
			}
		}
