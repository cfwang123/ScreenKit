...
与 ScreenKit 电脑端配套的安卓 App。当前版本：**1.0.10**（随 ScreenKit 1.0.10 发布）。安装后桌面有两个入口：**传文件**、**投屏**。
...
7. 桌面 **投屏**：扫描同一 Wi‑Fi 上的电脑（或填 IP / USB 投屏），允许截屏后把画面推到 ScreenKit。画质三档 540p/720p/1080p，可关声音。电脑需保持投屏接收（默认开）。USB 需开 USB 调试，由电脑 `adb reverse`。
...
- 投屏：UDP 19518 发现（`app=screencast`）；TCP 19519 或 USB `adb reverse` / Accessory，帧 `SCST` + H.264 Annex-B + AAC ADTS
