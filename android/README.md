...
1. 电脑打开 ScreenKit 主界面 **文件同步** Tab（参数设置 → 接口里启用「PC 文件传输」，默认开）。点 **安装到手机** 显示本机局域网下载地址和二维码（`http://<电脑IP>:17532/apk`），手机与电脑同一局域网，用浏览器扫码下载安装。PC 编译会把本目录最新 APK 拷到程序旁 `apk/`（仅当源更新）。
