...
7. 菜单 **投屏**（或主页 **投屏** 按钮）：带上当前已选/已连接电脑。进入后扫描同一 Wi‑Fi 上的电脑；也可点 **我的电脑** 或列表点选。**开始投屏** 用已选电脑或手动 IP，不必列表保持勾选。手机发 hello、收到电脑 hello 后才显示「投屏中」并开始采集。**USB 投屏**：电脑必须已开 ScreenKit；配件通道读写带短超时，避免电脑弹窗后一直黑屏。电脑未开或未应答 hello 时提示「电脑未打开 ScreenKit」。无配件时约 90 秒改走 USB 网络共享。**USB 投屏(adb)**：需要 USB 调试。纯 USB 测试画面：`am start …/.CastActivity --ez scst_usb_pat true`。
