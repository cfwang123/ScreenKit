...
7. 桌面 **投屏**：打开即扫描同一 Wi‑Fi 上的电脑。状态显示 **wifi / usb / usb(adb)**。横屏会按当前画质重编码。**USB 投屏**：纯 USB 配件（AOA），不用网络、不用 USB 调试；电脑先在投屏设置里点 **USB配件**，再在手机上点 **USB 投屏** 并允许配件。**USB 投屏(adb)**：需要 USB 调试。
