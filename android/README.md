...
7. 桌面 **投屏**：扫描同一 Wi‑Fi 上的电脑（或填 IP），允许截屏后把画面推到 ScreenKit。画质三档 540p/720p/1080p，可关声音。电脑需保持投屏接收（默认开）。**USB 投屏**（**不用 USB 调试**）：插上数据线。电脑若把手机切成配件，允许后点 **USB 投屏**。切不成配件时，通知栏把 USB 用途设为 **网络共享**，再点 **USB 投屏**。
