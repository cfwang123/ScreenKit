...
1. 电脑打开 ScreenKit 主界面 **文件同步** Tab（参数设置 → 接口里启用「PC 文件传输」，默认开）。点 **安装到手机** 显示本机局域网下载地址和二维码（`http://<电脑IP>:17532/apk`，多网卡时默认用能连外网的地址），手机与电脑同一局域网，用浏览器扫码下载安装。也可用浏览器打开同一端口的 **网页管理**（电脑 `/`、手机 `/m`，需登录才能上传）。PC 编译会把本目录最新 **release** APK 拷到程序旁 `apk/`（仅当源更新；不用 debug）。
...
7. 桌面 **投屏**：打开即扫描同一 Wi‑Fi 上的电脑。状态显示 **wifi / usb / usb(adb)**。横屏会按当前画质重编码。**USB 投屏**：电脑点 **USB配件** 把手机切到 AOA（配件口被 spacedesk 占用时电脑会换成 WinUSB），手机可因 `USB_ACCESSORY_ATTACHED` 自动开始；不用 adb reverse。约 90 秒无配件则改走 USB 网络共享。**USB 投屏(adb)**：需要 USB 调试。
