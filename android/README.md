...
7. 桌面 **投屏**：打开即扫描同一 Wi‑Fi 上的电脑（或填 IP），允许截屏后把画面推到 ScreenKit。画质三档 540p/720p/1080p，可关声音。电脑需保持投屏接收（默认开）。**USB 投屏**（不用 USB 调试）：插上数据线，配件或通知栏 **网络共享**。**USB 投屏(adb)**：开 USB 调试，电脑 ScreenKit 会做 `adb reverse`，再点此按钮。
