...
与 ScreenKit 电脑端配套的安卓 App。当前版本：**1.0.10**（随 ScreenKit 1.0.10 发布）。桌面仅一个入口：**传文件/投屏**（英文系统显示 **ScreenKit**）；**投屏** 在 App 内标题栏菜单进入，与传文件同一任务栈。
...
2. 安装本 App 后启动：单页完成选电脑与传文件。自动连接**上次成功的电脑**；失败则点 **我的电脑** 打开居中弹窗从已保存列表选择（可设别名、删除）、或搜索局域网、手填 IP。投屏页共用同一 **我的电脑** 弹窗。
3. 首次连接：手机弹出「等待电脑确认」（可取消）；电脑弹窗始终置顶，点允许后手机等待窗消失。
4. 主界面自上而下：**连接状态**（已连接可点「更换」展开选电脑）、**上传文件** / **拍照上传** / **网页文件管理**（按钮按内容宽度自动换行）、**接收文件夹**（同页可绑定）、**文本同步**（发送 + 只读显示电脑最新一条）、**传输记录**（进度与完成；来自电脑的文件可点开用系统应用打开）。**参数设置** 可配置拍照压缩：JPG/PNG、JPG 质量（默认 60%）、最长边限制（默认 2000px，可关闭）。
5. 连接电脑后点 **网页文件管理**（或标题栏菜单同名项）用系统浏览器打开 `http://<电脑IP>:1224/m`。标题栏三横菜单：**投屏**、**网页文件管理**、**参数设置**、**检查更新**（投屏页左上角返回传文件）。未绑定接收文件夹则无法接收电脑拖来的文件（参数设置里也可绑定）。
6. 系统分享文件到本 App（相册/文件管理可一次多选）：直接传到电脑 `sendfile/`，并出现在传输列表；先直连上次电脑，失败再选电脑，选完自动发完。
7. 菜单 **投屏**：进入后扫描同一 Wi‑Fi 上的电脑（插 USB 时扫描绑 Wi‑Fi 口，并向上次电脑 IP 单播）。手机发 hello、收到电脑 hello 后才显示「投屏中」并开始采集；电脑回 hello 后才弹窗。电脑画面窗 Tab 显示类型 **网络 / usb / adb**。横屏会按当前画质重编码。停止投屏时向电脑发 `GET /api/cast/stop` 立刻关窗。**USB 投屏**：电脑必须已开 ScreenKit（开接收会自动配件助手，也可点 **USB配件**）；电脑未开或未应答 hello 时提示「电脑未打开 ScreenKit」，不会显示投屏中。配件口被 spacedesk 占用时电脑会换成 WinUSB（不对 spacedesk `Open`）；无配件时会 `adb kill-server` 再发 AOA。约 90 秒无配件则改走 USB 网络共享。**USB 投屏(adb)**：需要 USB 调试。纯 USB 测试画面（不截屏）：`am start …/.CastActivity --ez scst_usb_pat true`。
