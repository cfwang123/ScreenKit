...
7. 桌面 **投屏**：打开即扫描同一 Wi‑Fi 上的电脑（手动 IP 会自动填入连上的地址）。状态显示类型 **wifi / usb / usb(adb)**。允许截屏后把画面推到 ScreenKit。画质三档 540p/720p/1080p，可关声音。电脑需保持投屏接收（默认开）。**USB 投屏**（不用 USB 调试）：通知栏把 USB 设为 **网络共享**，或走 USB 配件。**USB 投屏(adb)**：需要 USB 调试，与上一按钮分开。
