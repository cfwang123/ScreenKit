...
public partial class MainWindow {
	...
	void applyvoicehudmsg(string s) {
		...
		if (s.StartsWith("润色中")) {
			var orig = s;
			if (orig.StartsWith("润色中…")) orig = orig.Substring("润色中…".Length);
			else if (orig.StartsWith("润色中")) orig = orig.Substring("润色中".Length);
			orig = orig.Trim().TrimStart('·', ' ').Trim();
			if (orig.Length > 0)
				asrVoiceHud.SetPolish(orig);
			return;
		}
		if (s.StartsWith("识别中")) {
			asrVoiceHud.SetDetail("识别中 · Esc 停止", "");
			return;
		}
		if (s.StartsWith("已中止")) {
